...
function renderInsights() {
    ...
    // Stale check: the digest describes one fixed week; flag it once the data
    // has moved on (it is regenerated by weekly_sync.sh, but can lag/fail).
    const latestWeek = Array.isArray(allTrendData) && allTrendData.length
        ? allTrendData[allTrendData.length - 1].week : null;
    const stale = !!(latestWeek && insights.weekLabel && insights.weekLabel !== latestWeek);
    const sub = document.getElementById('insights-sub');
    if (sub) {
        const base = insights.weekLabel
        ...
        sub.textContent = stale
            ? `${base} ⚠ Veraltet — aktuelle Datenwoche ist ${latestWeek}.`
            : base;
        sub.classList.toggle('insights-stale', stale);
    }
...
function fillInsightsList(ul, items, kind) {
    ...
    // Honor the category filter like every other panel (skip before the
    // first week has populated activeCategories).
    if (activeCategories && activeCategories.size > 0) {
        items = items.filter(d => !d.cat || activeCategories.has(d.cat));
    }
    if (!items.length) {
        setEmpty(ul, isPricier ? 'Keine teureren Artikel (in den aktiven Kategorien).' : 'Keine besonderen Deals (in den aktiven Kategorien).');
        return;
    }
